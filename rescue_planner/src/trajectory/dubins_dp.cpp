...
struct Cell {
    double th = 0.0;          // candidate heading
    double len = kInf;        // best downstream length from here to the end
    int next = -1;            // index of chosen cell in the next column
};
}  // namespace
...
std::vector<double> optimizeHeadings(const std::vector<Vec2>& pts,
    ...
    // Build candidate headings per waypoint.
    ...
    for (int i = 0; i < P; ++i) {
        if (i == 0) {
            ...
            // Seed with geometric tangent guesses (prev->cur and cur->next).
            const double th_in =
    ...
    // Terminal column: cost 0 (nothing left to traverse).
    for (auto& c : col[P - 1]) c.len = 0.0;

    // Backward DP: fill column idx using column idx+1.
    ...
    // Trace the optimal chain forward from the (single) start cell.
    int best = 0;
    double bestLen = kInf;
    ...
    for (int idx = 0; idx < P; ++idx) {
        ...
        if (cur < 0) {  // chain broke (shouldn't with valid Dubins); fall back
